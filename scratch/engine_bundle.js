...
function runSimulation(cohortSize = 100, customInterventions, courses = INITIAL_COURSES) {
  const interventions = {
  ...
  for (let sem = 1; sem <= MAX_SEMESTERS; sem++) {
    ...
    for (const student of students) {
      ...
      eligibleCourses.sort((a, b) => {
        const weightA = (a.category === "Major" ? 100 : 0) + (downstreamWeights.get(a.classId) || 0) * 10;
        const weightB = (b.category === "Major" ? 100 : 0) + (downstreamWeights.get(b.classId) || 0) * 10;
      ...
      if (interventions.enableSpringSummer && currentTerm === "Winter" && !student.isGraduated) {
        ...
        for (const c of coursesMap.values()) {
          ...
          if (isNonMajor || isSpringSummerOffered) {
            if (c.category === "Gen") ssEligible.push(c);
            ...
            else if (c.category === "EPSEL" && ssEpselDone < interventions.epselCreditsRequired) ssEligible.push(c);
